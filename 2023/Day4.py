...
def part1():
    ...
    results = sum(int(2**(len(set.intersection(*map(lambda x: set(x.split()), l[8:].split(" | "))))-1)) for l in input_data.split('\n'))
    return results
...
def part2():
    ...
    for n, l in enumerate(lines, 1):
        matches = len(set.intersection(*map(lambda x: set(x.split()), l[8:].split(" | "))))
