...
def part1():
    ...
    for room in input_data:
        ...
        i = sorted([(c, enc_room.count(c)) for c in set(sorted(enc_room.strip())) if c != '-'], key=lambda x: -x[1]*100+ord(x[0]))[:5]
...
def part2():
    ...
    for room in input_data:
        ...
        if all(i[x][0] == checksum[x] for x in range(5)):
            room = ''.join([ascii_lowercase[(ascii_lowercase.find(c)+int(room_id)%26)%26] if c != '-' else ' ' for c in enc_room])
            # print(room, room_id)
