...
def part1():
    ...
    for r, c in zip(rx, cx):
        try:
            result += checkpat(
                dfs, [r, r, r, r], [c + 0, c + 1, c + 2, c + 3], [*"XMAS"]
            )
            result += checkpat(
                dfs, [r + 0, r + 1, r + 2, r + 3], [c, c, c, c], [*"XMAS"]
            )
            result += checkpat(
                dfs,
                [r + 0, r + 1, r + 2, r + 3],
            ...
            result += checkpat(
                dfs, [r - 3, r - 2, r - 1, r], [c - 3, c - 2, c - 1, c], [*"SAMX"]
            )
            result += checkpat(
                dfs, [r - 3, r - 2, r - 1, r], [c + 3, c + 2, c + 1, c], [*"SAMX"]
            )
...
def part2():
    a = np.array([[*l] for l in input_data.splitlines()])
    df = pd.DataFrame(a)
    df_check = pd.DataFrame([[*"M.S"], [*".A."], [*"M.S"]])
    r_df, c_df = df.shape
    result = 0
    for r in range(r_df):
        for c in range(c_df):
            ...
            if sub_df.size == 9:
                result += max(
                    np.diag(np.rot90(sub_df.to_numpy(), k) == df_check.to_numpy()).all()
                    & np.diag(
                        np.rot90(np.rot90(sub_df.to_numpy(), k) == df_check.to_numpy())
                    ).all()
                    for k in range(4)
                )
    return result
