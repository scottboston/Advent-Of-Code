...
def part1():
    ...
    def get_distance(t, flight_dur, flight_speed, rest_dur):
        completed = (
            (t // (flight_dur + rest_dur) + 1) * flight_dur * flight_speed
            if t % (flight_dur + rest_dur) > flight_dur
            else t // (flight_dur + rest_dur) * flight_dur * flight_speed
        )
        flying_now = (
            t % (flight_dur + rest_dur) * flight_speed
            if t % (flight_dur + rest_dur) < flight_dur
            else 0
        )
...
def part2():
    ...
    for i in range(1, 2_504):
        d = np.array(
            [
                get_distance(i, *map(int, e))
                for n, e in enumerate(map(find_all, input_data.splitlines()))
            ]
        )
